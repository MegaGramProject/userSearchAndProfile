...
const imageGridForLikedPosts = document.getElementById('imageGridForLikedPosts');
const imageGridForLikedReels= document.getElementById('imageGridForLikedReels');
const noLikedPosts = document.getElementById('noLikedPosts');
const noLikedReels = document.getElementById('noLikedReels');
...
let listOfLikedPosts = [];
let listOfLikedReels = [];
let userBlockings = [];
...
async function authenticateUserAndFetchData() {
    ...
    const response2 = await fetch('http://localhost:8004/getLikedPostsOfUser/'+authenticatedUsername);
    if(!response2.ok) {
        throw new Error('Network response not ok');
    }
    const listOfLikedPostIds = await response2.json();
    
    if(listOfLikedPostIds.length>0) {
        ...
        for(let postId of listOfLikedPostIds) {
            let postInfo = {};
            if(postId in postIdToPostInfoMappings && postIdToPostInfoMappings[postId]['smallestSlideNumber']==0) {
                ...
                postInfo['postId'] = postId;
                if(!postInfo['hasMoreThanOneSlide']) {
                    if(postId in listOfLikedPostIds) {
                        postInfo['hasMoreThanOneSlide'] = true;
                    }
                }
            }
            
            listOfLikedPosts.push(postInfo);
        }
        ...
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            listOfPostIds: listOfLikedPostIds
        })
        });
        if(!response5.ok) {
            throw new Error('Network response not ok');
        ...
        for(let post of listOfLikedPosts) {
        ...
        listOfLikedPosts.sort((a, b) => new Date(b['dateTimeOfPost']) - new Date(a['dateTimeOfPost']));
        
        createDOMElementsForLikedPosts();
        
    }
    else {
        noLikedPosts.classList.remove('hidden');
        imageGridForLikedPosts.classList.add('hidden');
    }
}
...
function showLikedPosts() {
    if(currentlyShownSection!=='POSTS') {
        if(currentlyShownSection==='REELS') {
            ...
            imageGridForLikedReels.classList.add('hidden');
            noLikedReels.classList.add('hidden');
        }
        ...
        likedHeader.textContent = 'Liked Posts';

        if(listOfLikedPosts.length>0) {
            imageGridForLikedPosts.classList.remove('hidden');
        }
        else {
            noLikedPosts.classList.remove('hidden');
        }
...
function showLikedReels() {
    if(currentlyShownSection!=='REELS') {
        if(currentlyShownSection==='POSTS') {
            ...
            imageGridForLikedPosts.classList.add('hidden');
            noLikedPosts.classList.add('hidden');
        ...
        likedHeader.textContent = 'Liked Reels';

        if(listOfLikedReels.length>0) {
            imageGridForLikedReels.classList.remove('hidden');
        }
        else {
            noLikedReels.classList.remove('hidden');
        }
    }
}
...
function createDOMElementsForLikedPosts() {
    let counter = 0;
    let index=0;
    ...
    for(let post of listOfLikedPosts) {
        ...
        index++;
        if(counter>3) {
            imageGridForLikedPosts.appendChild(newRowDiv);
            counter=1;
